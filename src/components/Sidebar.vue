...
const activeIndicatorClass = "before:content-[' '] before:rounded-lg before:w-2 before:-translate-x-1 before:-translate-y-1/2 before:top-1/2 before:block before:absolute before:h-[80%] before:m-auto before:bg-white overflow-hidden";
...
<template>
  <div class="flex h-full w-20 overflow-y-auto bg-discord-dark-3">
    <div class="flex flex-col items-center gap-4 pt-4">
      <div
        ...
        <div class="px-3">
          <img class="duration-50 h-[56px] w-[56px] rounded-2xl outline-lime-200 duration-300 hover:rounded-3xl" :class="image.active ? '' : 'rounded-5xl'" :src="image.url" alt="" />
        </div>
      </div>
    </div>
  </div>
